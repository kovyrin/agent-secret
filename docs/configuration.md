...
tokens.
GCP profile blocks store identity metadata only. Config files must never contain
resolved secret values, provider access tokens, refresh tokens, OAuth client
secrets, or service account keys.
