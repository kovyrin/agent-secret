...
func markGrantPayloadDelivered(b *Broker, correlation protocol.Correlation) error {
	if err := b.grants.completePayloadWrite(correlation, true); err != nil {
		b.removeActiveExec(correlation.RequestID)
		return err
	}
	return b.markPayloadDelivered(correlation.RequestID)
}
...
func TestBrokerApprovesBeforeResolvingAndAuditsBeforePayload(t *testing.T) {
	...
	grant, err := broker.handleExec(context.Background(), testCorrelation("req_1", "nonce_1"), req)
	if err != nil {
...
func assertDeadlineApprovalFailureAudited(t *testing.T, tc deadlineApprovalFailureCase) {
	...
	_, err = broker.handleExec(context.Background(), testCorrelation("req_1", "nonce_1"), req)
...
func assertApprovalFailureAudited(t *testing.T, tc approvalFailureAuditCase) {
	...
	_, err := broker.handleExec(context.Background(), testCorrelation("req_1", "nonce_1"), testExecRequest(t, []request.SecretSpec{
...
func TestBrokerDeduplicatesRefsAndPreservesEmptyValues(t *testing.T) {
	...
	grant, err := broker.handleExec(context.Background(), testCorrelation("req_1", "nonce_1"), testExecRequest(t, []request.SecretSpec{
		{Alias: "TOKEN", Ref: ref},
...
func TestBrokerReusableApprovalUsesCacheAndForceRefreshRefetches(t *testing.T) {
	...
	first, err := broker.handleExec(context.Background(), testCorrelation("req_1", "nonce_1"), req)
	if err != nil {
	...
	if err := markGrantPayloadDelivered(broker, testCorrelation("req_1", "nonce_1")); err != nil {
		t.Fatalf("markPayloadDelivered returned error: %v", err)
	}

	resolver.values[ref] = "second"
	second, err := broker.handleExec(context.Background(), testCorrelation("req_2", "nonce_2"), req)
	if err != nil {
	...
	if err := markGrantPayloadDelivered(broker, testCorrelation("req_2", "nonce_2")); err != nil {
		t.Fatalf("markPayloadDelivered returned error: %v", err)
	}

	force := req
	force.ForceRefresh = true
	third, err := broker.handleExec(context.Background(), testCorrelation("req_3", "nonce_3"), force)
	if err != nil {
...
func TestBrokerReusableApprovalUsesApprovedUseLimit(t *testing.T) {
	...
	third, err := broker.handleExec(context.Background(), testCorrelation("req_3", "nonce_3"), req)
	if err != nil {
...
func TestBrokerReservesReusableUseBeforePayloadDelivery(t *testing.T) {
	...
	_, err = broker.handleExec(context.Background(), testCorrelation("req_3", "nonce_3"), req)
...
func TestBrokerClearsReusableCacheOnExpiry(t *testing.T) {
	...
	second, err := broker.handleExec(context.Background(), testCorrelation("req_2", "nonce_2"), next)
	if err != nil {
		t.Fatalf("second HandleExec returned error: %v", err)
	}
...
func TestBrokerRejectsReusableApprovalThatExpiresDuringForceRefresh(t *testing.T) {
	...
	_, err = broker.handleExec(context.Background(), testCorrelation("req_2", "nonce_2"), force)
	if !errors.Is(err, ErrRequestExpired) {
...
func TestBrokerRejectsReusableApprovalThatExpiresBeforePayloadDelivery(t *testing.T) {
	...
	_, err = broker.handleExec(context.Background(), testCorrelation("req_2", "nonce_2"), reuse)
	if err != nil {
		t.Fatalf("reuse HandleExec returned error before payload delivery: %v", err)
	}

	now = req.ExpiresAt.Add(time.Second)
	if err := markGrantPayloadDelivered(broker, testCorrelation("req_2", "nonce_2")); !errors.Is(err, ErrRequestExpired) {
...
func TestBrokerRollsBackReusableApprovalWhenCacheInsertFails(t *testing.T) {
	...
	grant, err := broker.handleExec(context.Background(), testCorrelation("req_2", "nonce_2"), req)
	if err != nil {
...
func TestBrokerReportLifecycleValidatesNonceAndAudits(t *testing.T) {
	...
	if _, err := broker.handleExec(context.Background(), testCorrelation("req_1", "nonce_1"), req); err != nil {
		t.Fatalf("HandleExec returned error: %v", err)
	}
